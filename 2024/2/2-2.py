...
def error_correct_level(level):
    #start = copy.deepcopy(level)
    
    for index in range(len(level)):
        current = copy.deepcopy(level)
        del current[index]
        if is_level_safe(current) == 'safe':
            return "safe"
    return "unsafe"
...
def one(args, data):
    ...
    for level in data:
        ...
        debug_line = ''.join(str(n) for n in level) + ' ' + safety_status
        if safety_status != 'safe':
            safety_status = error_correct_level(level)
            debug_line += ' [ec '
            debug_line += 'success' if safety_status == 'safe' else 'fail'
            debug_line += ']'
        if safety_status == 'safe':
            safe_reports += 1
        if args.debug:
